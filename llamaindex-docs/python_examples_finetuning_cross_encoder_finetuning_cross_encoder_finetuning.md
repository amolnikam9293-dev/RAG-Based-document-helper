...
for index inrange(0,(sections_list)):
...
print(f"Number of papers in the test sample:- (df_test)}")
...
print(f"Number of papers in the reranking eval dataset:- (df_reranking)}")
...
for index inrange(0,(query_list)):
...
evaluator_gpt4_pairwise =PairwiseComparisonEvaluator=gpt4)
